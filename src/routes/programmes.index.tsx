...
export const Route = createFileRoute("/programmes/")({
  head: () => ({
    meta: [
      { title: "Programmes — VenueHub" },
      { name: "description", content: "Browse, filter and manage college programmes." },
    ],
  }),
...
function ProgrammesContent() {
  ...
  const filtered = useMemo(() => {
    return scoped.filter((p) => {
      ...
      if (q && !`${p.name} ${p.wing} ${p.category}`.toLowerCase().includes(q.toLowerCase())) return false;
      return true;
    });
        ...
        {filtered.length === 0 ? (
          <div className="p-6">
            <EmptyState
              ...
              action={user.role === "wing" ? { label: "Register Programme", onClick: () => navigate({ to: "/programmes/new" }) } : undefined}
            />
          </div>
        ) : (
          <>
