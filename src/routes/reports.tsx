...
export const Route = createFileRoute("/reports")({
  head: () => ({
    meta: [
      { title: "Reports — VenueHub" },
      { name: "description", content: "Programme, venue and budget reports." },
    ],
  }),
  component: () => (
    <AppShell>
...
function ReportsPage() {
  ...
  const catData = Object.entries(catUsage).filter(([, v]) => v > 0).map(([name, value]) => ({ name, value }));
  ...
  const approvalStats = [
    ...
    { label: "Pending", value: programmes.filter((p) => ["submitted", "union_approved"].includes(p.status)).length },
  ];

  const colors = ["var(--primary)", "var(--success)", "var(--warning)", "var(--destructive)", "var(--info)", "oklch(0.6_0.15_320)", "oklch(0.68_0.16_45)"];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card title="Venue Usage" subtitle="Number of programmes per venue">
          <ResponsiveContainer>
            <BarChart data={venueData} layout="vertical" margin={{ left: 20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" horizontal={false} />
              <XAxis type="number" stroke="var(--muted-foreground)" fontSize={12} tickLine={false} axisLine={false} />
              <YAxis dataKey="name" type="category" stroke="var(--muted-foreground)" fontSize={12} tickLine={false} axisLine={false} width={120} />
        ...
        <Card title="Programme Categories" subtitle="Distribution across categories">
          <ResponsiveContainer>
            <PieChart>
        ...
        <Card title="Budget by Category" subtitle="Total budget (₹) per category" className="lg:col-span-2">
          <ResponsiveContainer>
