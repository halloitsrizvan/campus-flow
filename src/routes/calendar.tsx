...
export const Route = createFileRoute("/calendar")({
  head: () => ({
    meta: [
      { title: "Venue Calendar — VenueHub" },
      { name: "description", content: "Monthly calendar view of programmes and venue bookings." },
    ],
  }),
  component: () => (
    <AppShell>
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
        ...
        <div className="grid grid-cols-7">
          {grid.map((day, i) => {
            const dayProgrammes = programmes.filter((p) => isSameDay(new Date(p.date), day) && p.status !== "rejected");
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  {dayProgrammes.length > 3 && (
                    <div className="text-[11px] text-muted-foreground">+{dayProgrammes.length - 3} more</div>
                  )}
                </div>
              </div>
            );
      ...
      <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
        ...
        {Object.entries(CATEGORY_COLORS).map(([cat, cls]) => (
          <span key={cat} className={cn("inline-flex items-center rounded-full border px-2 py-0.5", cls)}>{cat}</span>
        ))}
      </div>
          ...
          {programme && (
            <>
              <SheetHeader>
                ...
                <SheetDescription>{programme.wing} · {programme.category}</SheetDescription>
              </SheetHeader>
              <div className="mt-6 space-y-4 text-sm">
                <Row label="Date">{format(new Date(programme.date), "PPP")}</Row>
                <Row label="Time">{programme.startTime}–{programme.endTime}</Row>
                ...
                <Row label="Status"><StatusBadge status={programme.status} /></Row>
                <p className="text-muted-foreground">{programme.purpose}</p>
                <Button asChild className="w-full">
                  <Link to="/programmes/$id" params={{ id: programme.id }}>Open programme →</Link>
                </Button>
              </div>
            </>
          )}
