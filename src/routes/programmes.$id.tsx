...
export const Route = createFileRoute("/programmes/$id")({
  head: ({ params }) => ({
    meta: [
      { title: `Programme #${params.id} — VenueHub` },
      { name: "description", content: "Programme details, approval timeline and comments." },
    ],
  }),
  component: () => (
    <AppShell>
...
function ProgrammeDetail() {
  ...
  if (!programme) {
    return (
      <div className="mx-auto max-w-2xl rounded-xl border bg-card p-10 text-center">
        ...
        <Button className="mt-4" onClick={() => navigate({ to: "/programmes" })}>Back to programmes</Button>
      </div>
    );
  }
    ...
    if (user!.role === "union") {
      updateProgramme(programme!.id, {
        ...
        timeline: programme!.timeline.map((t) =>
          t.label.toLowerCase().includes("union") ? { ...t, done: true, at: new Date().toISOString() } : t,
        ),
      });
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <button onClick={() => navigate({ to: "/programmes" })} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-4">
          <div className="rounded-xl border bg-card p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-muted-foreground whitespace-pre-wrap">{programme.purpose}</p>
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <Info icon={<CalendarDays className="h-4 w-4" />} label="Date & Time" value={`${format(new Date(programme.date), "PPP")} · ${programme.startTime}–${programme.endTime}`} />
              <Info icon={<MapPin className="h-4 w-4" />} label="Venue" value={venueName(programme.venueId)} />
              <Info icon={<Users className="h-4 w-4" />} label="Expected Students" value={String(programme.expectedStudents)} />
              <Info icon={<Wallet className="h-4 w-4" />} label="Budget" value={`₹${programme.budget.toLocaleString()}`} />
              {programme.guest && <Info icon={<Users className="h-4 w-4" />} label="Chief Guest" value={programme.guest} />}
              {programme.equipment && <Info icon={<Paperclip className="h-4 w-4" />} label="Equipment" value={programme.equipment} />}
            </div>
          </div>
                ...
                <div key={c.id} className="flex gap-3">
                  ...
                  <div className="flex-1 rounded-lg border bg-background p-3">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="text-xs text-muted-foreground">{format(new Date(c.at), "MMM d, HH:mm")}</div>
                    </div>
              ...
              <div className="mt-3 flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((s) => (
                  <button key={s} onClick={() => setRating(s)}>
                    <Star className={cn("h-6 w-6", s <= rating ? "fill-warning text-warning" : "text-muted-foreground")} />
                  </button>
                ))}
              </div>
              <Textarea className="mt-3" value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Remarks…" rows={2} />
              <Button className="mt-3" size="sm" onClick={submitRating}>Submit rating</Button>
            </div>
          )}
        </div>
              ...
              {programme.timeline.map((t, i) => (
                <li key={i} className="flex gap-3">
                  <div className={cn(
                    ...
                    t.done ? "border-success bg-success text-success-foreground" : "border-border bg-muted text-muted-foreground",
                  )}>
                    {t.done ? <Check className="h-3 w-3" /> : i + 1}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className={cn("text-sm", t.done ? "font-medium" : "text-muted-foreground")}>{t.label}</div>
                    {t.at && <div className="text-xs text-muted-foreground">{format(new Date(t.at), "MMM d, yyyy")}</div>}
                  </div>
                </li>
              ))}
            </ol>
          ...
          {canApprove && (
            <div className="rounded-xl border bg-card p-6 shadow-sm">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {user.role === "union" ? "Review and approve for faculty sign-off." : "Provide final approval and confirm booking."}
              </p>
          ...
          {programme.rating && (
            <div className="rounded-xl border bg-card p-6 shadow-sm">
              ...
              <div className="mt-2 flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((s) => (
                  <Star key={s} className={cn("h-5 w-5", s <= (programme.rating ?? 0) ? "fill-warning text-warning" : "text-muted-foreground")} />
                ))}
                <span className="ml-2 text-sm font-medium">{programme.rating?.toFixed(1)}</span>
              </div>
              {programme.ratingRemarks && <p className="mt-2 text-sm text-muted-foreground">{programme.ratingRemarks}</p>}
            </div>
          )}
        </div>
      </div>
